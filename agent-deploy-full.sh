#!/usr/bin/env bash
# ============================================================
# Urartu — деплой сайта-ОС + голосового агента на VPS (dev-режим).
# Ставит: nginx-статику сайта (клон публичного репо), FastAPI-агента
# (kie.ai LLM + edge-tts) и Caddy с автоматическим HTTPS через sslip.io
# (временный домен для разработки — потом переключим на свой домен).
# Запуск НА СЕРВЕРЕ под root:
#   KIE_API_KEY=ваш_ключ bash deploy-full.sh
# ============================================================
set -e
APP=/opt/urartu-agent
SITE=/var/www/urartu-ai
REPO="https://github.com/Shahen777/urartu-ai.git"
...
apt-get update -qq
apt-get install -y -qq python3-venv python3-pip git curl debian-keyring debian-archive-keyring apt-transport-https >/dev/null

# Caddy (авто-HTTPS, отдаёт и статику сайта, и проксирует /api/* на агента)
...
echo "→ Caddy: сайт + прокси на агента ($HOST)…"
...
cat > /opt/urartu-agent/update.sh <<'UPD'
#!/usr/bin/env bash
set -e
git -C /var/www/urartu-ai pull --ff-only
systemctl restart urartu-agent
echo "Обновлено: сайт подтянут, агент перезапущен."
UPD
chmod +x /opt/urartu-agent/update.sh

sleep 4
echo ""
echo "============================================================"
echo "✅ Готово (dev-режим, временный домен):"
echo "   Сайт:  https://$HOST"
echo "   Агент: https://$HOST/api/health"
echo ""
echo "Когда решите переключить постоянный домен через reg.ru:"
echo "  1) A-запись домена → $IP"
echo "  2) На сервере: заменить $HOST на ваш домен в /etc/caddy/Caddyfile, затем: systemctl reload caddy"
echo "Обновить сайт/агента позже:  bash /opt/urartu-agent/update.sh"
echo "============================================================"
